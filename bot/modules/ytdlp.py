...
async def _ytdl(client, message, isLeech=False, sameDir=None):
    ...
    # TikTok cookie check will happen after quality selection - removed early check
    ...
    try:
        ...
        # Check if TikTok error related to cookies/authentication
        if "tiktok" in link.lower() and ("Unable to extract" in error_msg or "impersonate" in error_msg or "login" in error_msg.lower()):
            import os
            cookies_file = f"/usr/src/app/cookies_{user_id}.txt"
            main_cookies = "/usr/src/app/cookies.txt"

            # If no cookies exist, show login option
            if not os.path.exists(cookies_file) and not os.path.exists(main_cookies):
                from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton

                login_url = f"http://10.5.110.129/tiktok/login/{user_id}"
                buttons = InlineKeyboardMarkup([
                    [InlineKeyboardButton("🌐 Login to TikTok (Browser)", url=login_url)],
                    [InlineKeyboardButton("📋 Manual Cookie Setup", callback_data=f"tiktok_manual_{user_id}")]
                ])

                await sendMessage(
                    f"{tag}\n\n"
                    "🎵 <b>TikTok Login Required</b>\n\n"
                    "To download TikTok videos, you need to log in.\n\n"
                    "<b>Option 1 (Recommended):</b>\n"
                    "• Click the button below to open TikTok login\n"
                    "• Log in with your TikTok account\n"
                    "• Cookies will be automatically extracted\n"
                    "• Then retry your download!\n\n"
                    "<b>Option 2 (Manual):</b>\n"
                    "• Use /setcookies command with your cookies",
                    message,
                    buttons
                )
                __run_multi()
...
@new_task
async def setcookies(client, message):
    """Handle TikTok cookie setup"""
    user_id = message.from_user.id
    cookies_text = message.text.split(maxsplit=1)

    if len(cookies_text) < 2:
        help_msg = (
            "<b>🍪 Set TikTok Cookies</b>\n\n"
            "<b>Usage:</b> <code>/setcookies [cookies]</code>\n\n"
            "<b>Example:</b>\n"
            "<code>/setcookies sessionid=abc123; sid_tt=xyz456; tt_chain_token=token789</code>\n\n"
            "Or reply to a message containing cookies with <code>/setcookies</code>"
        )
        await sendMessage(help_msg, message)
        return

    cookies_str = cookies_text[1] if len(cookies_text) > 1 else ""

    # Check if replying to a message with cookies
    if not cookies_str and message.reply_to_message:
        cookies_str = message.reply_to_message.text

    if not cookies_str or len(cookies_str) < 50:
        await sendMessage("❌ Invalid cookies! Please provide valid TikTok cookies.", message)
        return
    ...
    try:
        ...
        with open(cookies_file, 'w') as f:
            ...
            f.write("# Generated by setcookies command\n\n")

            # Parse cookies (handle both formats: browser console or Netscape)
            if "\t" in cookies_str:
                # Already in Netscape format
                f.write(cookies_str)
            else:
                # Browser console format - convert to Netscape
        ...
        # Also copy to main cookies.txt
        import shutil
        shutil.copy(cookies_file, "/usr/src/app/cookies.txt")

        await sendMessage(
            f"✅ <b>Cookies Saved Successfully!</b>\n\n"
            f"You can now use <code>/ytdl</code> to download TikTok videos.\n\n"
            f"<b>Example:</b>\n<code>/ytdl https://www.tiktok.com/@username</code>",
            message
        )

        LOGGER.info(f"TikTok cookies set for user {user_id}")

    except Exception as e:
        await sendMessage(f"❌ Error saving cookies: {str(e)}", message)
        LOGGER.error(f"Error in setcookies: {e}")
